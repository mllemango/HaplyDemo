...
void setup() {
  ...
  obs2 = new FBox(2, 2);
  obs3 = new FBox(4, 4);
  obs1.attachImage(loadImage("img/leaf3.png"));
  obs2.attachImage(loadImage("img/leaf3.png"));
  obs3.attachImage(loadImage("img/greenbin.png"));
...
/*    if (s.h_avatar.isTouchingBody(square_light)){
    ...
    */
    ...
    //println(obs2.getY());
/*    if (obs2.getY() > 8.2 && check2 == 0) {
      obs2.setPosition(worldWidth-5, 1);
      obs2.setVelocity(0,0.02);
    } 
    
    //println(obs1.getY());
    if (obs1.getY() > 8.2 && check1 == 0) {
      obs1.setPosition(worldWidth*1/3, 1);
